...
7. The weekly leaderboard ranks earned minerals, including minerals not yet claimed.
...
- The MVP does not include Colonize, XP, colony levels, manual special editions,
  referral payouts, or on-chain mineral claims.
- Mining, bonuses, balances, claims, and leaderboard calculations are off-chain and use
...
same-Type bonus change, claim, or weekly finalization. Minerals earned before a transfer
...
`min(now, periodEnd)`. Claiming minerals moves value into the spendable balance but never
changes the weekly score. A finalized week is reproducible from immutable ledger and
...
- A weekly leaderboard ranks earned minerals whether or not users claim them.
