...
function SummaryDialog({ count, total, ticketPrice, automaticQuickPick, disabled, approvalSpender, approvalAmount, onApproved, onBack, onLaunch }: { count: number; total: bigint; ticketPrice: bigint | undefined; automaticQuickPick: boolean; disabled: boolean; approvalSpender: `0x${string}`; approvalAmount: bigint; onApproved: () => void; onBack: () => void; onLaunch: () => void }) { return <div role="dialog" aria-modal="true" aria-labelledby="summary-title" className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4"><section className="w-full max-w-md rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5"><h2 id="summary-title" className="font-hud text-2xl font-semibold text-[var(--text-primary)]">Confirm expedition</h2><dl className="mt-5 space-y-3 text-sm"><Row label="Planets" value={count.toString()} /><Row label="Ticket price" value={ticketPrice ? <UsdcAmount value={ticketPrice} precision={2} /> : '—'} /><Row label="Total" value={<UsdcAmount value={total} precision={2} />} /><Row label="Coordinates" value={automaticQuickPick ? 'Automatic quick pick' : 'Manually selected'} /></dl><div className="mt-6 grid grid-cols-2 gap-2"><Button variant="secondary" onClick={onBack}>Back</Button><ApprovalButton spender={approvalSpender} amount={approvalAmount} onApproved={onApproved}><Button variant="primary" onClick={onLaunch} disabled={disabled} className="w-full">Confirm and launch</Button></ApprovalButton></div></section></div>; }
function Row({ label, value }: { label: string; value: ReactNode }) { return <div className="flex items-center justify-between border-b border-[var(--border)] pb-2"><dt className="text-[var(--text-secondary)]">{label}</dt><dd className="font-semibold text-[var(--text-primary)]">{value}</dd></div>; }
