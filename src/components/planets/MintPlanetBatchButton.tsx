  ...
  const mintBatch = async () => {
    ...
    try {
      ...
      if (availabilityCheck.unavailable.length > 0) {
        onUnavailable?.(
          availabilityCheck.unavailable.filter(({ reason }) => reason !== 'unreadable'),
        );
      }
      const unreadable = availabilityCheck.unavailable.filter(
      ...
      const liveTicketIds = new Set(
        availabilityCheck.live.map(({ ticketId }) => ticketId.toString()),
      );
      const livePlanets = validPlanets.filter(({ preview }) =>
        liveTicketIds.has(preview.descriptor.input.ticketId.toString()),
      );
      if (livePlanets.length === 0)
        throw new Error('No revealable ticket is currently owned by this wallet.');
  ...
  if (availability === 'wrong-chain')
    return (
      <p className="text-xs text-amber-200">
        Switch your wallet to Base Sepolia to reveal these planets.
      </p>
    );
