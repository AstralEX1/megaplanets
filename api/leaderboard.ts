...
/** Returns the UTC Monday-to-Monday leaderboard period containing the timestamp. */
export function getLeaderboardPeriod(now: Date): LeaderboardPeriodBounds {
  ...
  return {
    ...
    startsAt,
    endsAt: new Date(startsAt.getTime() + WEEK_MILLISECONDS),
  };
}

/** Sorts leaderboard rows deterministically and assigns one-based ordinal ranks. */
export function rankLeaderboardRows(rows: readonly UnrankedLeaderboardRow[]): RankedLeaderboardRow[] {
  return rows
    .map((row) => ({ ...row, walletAddress: normalizeWalletAddress(row.walletAddress) }))
    .sort((left, right) => {
      if (left.scoreMicros !== right.scoreMicros) return left.scoreMicros > right.scoreMicros ? -1 : 1;
...
export function getDistanceToNextRank(
  ...
  const index = rows.findIndex((row) => row.walletAddress === normalizeWalletAddress(walletAddress));
